...
Eigen::Matrix4f ICP::point_to_plane_solver(Frame & curr_frame, Frame & prev_frame, int iterations, bool cuda){
    ...
    for(int i = 0; i < iterations; i++){
        ...
        Eigen::Matrix<float, 6, 6> A = Eigen::MatrixXf::Zero(6, 6); // supposed to be ATA of the system according to paper
        ...
        cudaError_t cudaStatus0 = cudaMallocManaged(&dA, sizeof(Eigen::Matrix<float, 6, 6>));
        if(cudaStatus0 != cudaSuccess){
            std::cout << "Problem in CudaMalloc dA: " << cudaGetErrorString(cudaStatus0) << std::endl;
        }
        cudaStatus0 = cudaMemcpy(dA, A.data(), sizeof(Eigen::Matrix<float, 6, 6>), cudaMemcpyHostToDevice);
        if(cudaStatus0 != cudaSuccess){
            std::cout << "Problem in Cuda Copy of dA: " << cudaGetErrorString(cudaStatus0) << std::endl;
        }

        cudaStatus0 = cudaMallocManaged(&db, sizeof(Eigen::Matrix<float, 6, 1>));
        if(cudaStatus0 != cudaSuccess){
            std::cout << "Problem in CudaMalloc db: " << cudaGetErrorString(cudaStatus0) << std::endl;
        }
        cudaStatus0 = cudaMemcpy(db, b.data(), sizeof(Eigen::Matrix<float, 6, 1>), cudaMemcpyHostToDevice);
        if(cudaStatus0 != cudaSuccess){
            std::cout << "Problem in Cuda Copy of db: " << cudaGetErrorString(cudaStatus0) << std::endl;
        }
        
        cudaStatus0 = cudaMallocManaged(&curr_V_k, sizeof(Eigen::Vector3f) * curr_frame.V_k.size());
        if(cudaStatus0 != cudaSuccess){
        ...
        cudaStatus0 = cudaMemcpy(prev_N_gk, A.data(), sizeof(Eigen::Vector3f) * prev_frame.N_gk.size(), cudaMemcpyHostToDevice);
        if(cudaStatus0 != cudaSuccess){
            std::cout << "Problem in Cuda Copy of dA: " << cudaGetErrorString(cudaStatus0) << std::endl;
        }

        A_b_finder <<<block_num, thread_num>>>(
            dA, db, 
        ...
        cudaStatus0 = cudaMemcpy(A.data(), dA, sizeof(Eigen::Matrix<float, 6, 6>), cudaMemcpyDeviceToHost);
        if(cudaStatus0 != cudaSuccess){
            std::cout << "Problem in Copying A back to host: " << cudaGetErrorString(cudaStatus0) << std::endl;
        };
        cudaStatus0 = cudaMemcpy(b.data(), db, sizeof(Eigen::Matrix<float, 6, 1>), cudaMemcpyDeviceToHost);
        if(cudaStatus0 != cudaSuccess){
            std::cout << "Problem in Copying b back to host: " << cudaGetErrorString(cudaStatus0) << std::endl;
        };
        cudaFree(curr_V_k);
        cudaFree(curr_N_k);
        cudaFree(prev_V_gk);
